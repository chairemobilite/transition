...
import { addDraggingClass, removeDraggingClass } from '../MapCursorHelper';
...
const onPathSectionMapClick = async (e: MapMouseEvent) => {
    ...
    if (
        ...
        const selectedPathId = path.getId();
        if (clickedPathId !== selectedPathId) {
            // Clicked on a different path
            const hasUnsavedChanges = path.hasChanged() || (selectedLine && selectedLine.hasChanged());
            const paths = features.filter((feature) => feature.source === 'transitPaths');
            const clickedPath = features[clickedPathIndex];

            if (hasUnsavedChanges) {
                // Path or line has unsaved changes - show confirmation modal
                // After confirmation, select the clicked path directly (can't show context menu after modal)
                serviceLocator.eventManager.emit('map.showPathSwitchConfirmModal', () => {
                    // User confirmed - discard changes and select the clicked path
                    path.cancelEditing();
                    if (selectedLine) {
                        selectedLine.cancelEditing();
                    }
                ...
                if (paths.length === 1) {
                    ...
                    paths.forEach((pathGeojson) => {
                        const pathById = serviceLocator.collectionManager
                            .get('paths')
                            .getById(pathGeojson.properties?.id);
                        if (!pathById) return;
