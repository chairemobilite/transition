...
const onAccessMapMouseMove = (e: MapboxGL.MapMouseEvent) => {
    ...
    if (map._currentDraggingFeature === 'accessibilityMapLocation') {
        serviceLocator.eventManager.emit('routing.transitAccessibilityMap.dragLocation', e.lngLat.toArray(), true);
        e.originalEvent.stopPropagation();
    } else if (map._currentDraggingFeature === 'accessibilityMapLocation2') {
        serviceLocator.eventManager.emit('routing.transitAccessibilityMap.dragLocation', e.lngLat.toArray(), false);
        e.originalEvent.stopPropagation();
    }
};
