...
import { point as turfPoint, distance as turfDistance, featureCollection as turfFeatureCollection } from '@turf/turf';
...
const onMapRightClicked = (pointInfo: PointInfo, e: MjolnirEvent) => {
    const measureTool = serviceLocator.selectedObjectsManager.get('measureTool') as MeasureTool;
    if (measureTool && serviceLocator.selectedObjectsManager.isSelected('measureTool')) {
        serviceLocator.eventManager.emit('map.updateLayers', {
