...
class LayerSwitcherControl implements maplibregl.IControl {
    ...
    onAdd(map: maplibregl.Map): HTMLElement {
        ...
        this.button.setAttribute('data-testid', 'layer-switcher-button');

        // Create span container for the icon (same style as other controls)
        const iconSpan = document.createElement('span');
        iconSpan.className = 'maplibregl-ctrl-icon';
        iconSpan.setAttribute('aria-hidden', 'true');

        // Create the SVG icon for layers using DOM API
        ...
        // Create the path element for the layers icon
        const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        path.setAttribute('d', 'M10 3L2 7l8 4 8-4-8-4zM2 11l8 4 8-4M2 15l8 4 8-4');
        ...
        this.map.on('zoom', this.onZoomListener);
    ...
    private updateActiveState(): void {
        ...
        options.forEach((optionElement) => {
            ...
            // Only mark as active if the attribute value matches a known layer type
            const isActive = (layerType === 'osm' || layerType === 'aerial') && currentLayer === layerType;
            option.style.backgroundColor = isActive ? '#e8e8e8' : 'transparent';
        });
    }
    ...
    onRemove(): void {
        if (this.map) {
            this.map.off('zoom', this.onZoomListener);
        }
