...
class LayerSwitcherControl implements maplibregl.IControl {
    ...
    onAdd(map: maplibregl.Map): HTMLElement {
        ...
        // Add OSM option
        const osmOption = this.createOption('osm', this.t('main:map.controls.osm'));
        this.dropdown.appendChild(osmOption);

        // Add aerial option
        const aerialOption = this.createOption('aerial', this.t('main:map.controls.aerial'));
    ...
    private createOption(type: 'osm' | 'aerial', label: string): HTMLElement {
    ...
    private updateActiveState(): void {
        ...
        options.forEach((option) => {
            ...
            const type = element.dataset.layerType || element.getAttribute('data-layer-type');
            const isActive = type ? currentLayer === type : false;
            element.style.backgroundColor = isActive ? '#e8e8e8' : 'transparent';
        });
    }
