...
const TransitPathStatistics: React.FunctionComponent<PathStatsProps> = (props: PathStatsProps) => {
    ...
    // For rail modes, calculate curve-aware travel times dynamically
    const pathMode = path.getMode();

    // Extract mutable form values outside useMemo so they can be tracked as dependencies.
    // Fallback defaults come from lineModes config so each mode gets its own value.
    ...
    const maxSpeedKmH = path.getData('defaultRunningSpeedKmH', getDefaultRunningSpeedKmH(pathMode)) as number;

    const curveAwareStats = useMemo(() => {
        if (!isRailMode(pathMode)) {
            return null;
        }

        const analysis = analyzePathSegmentTravelTimes(path, {
        ...
        // Calculate "no dwell, no curves" time = pure running time at max speed (distance / speed)
        const runningSpeedMps = (maxSpeedKmH * 1000) / 3600; // Convert km/h to m/s
        const noDwellNoCurvesTimeSeconds = runningSpeedMps > 0 ? analysis.totalDistanceMeters / runningSpeedMps : 0;

        return {
            // "With dwell times" = curve-aware travel time WITH accel/decel at stops + dwell times
            operatingTimeWithoutLayoverTimeSeconds: analysis.totalTimeWithCurvesSeconds + totalDwellTimeSeconds,
            // "Without dwell times (with curves)" = running time at curve-limited speed (no stops)
            travelTimeWithoutDwellTimesSeconds: analysis.totalNoDwellCurveTimeSeconds,
            // "Without dwell times (without curves)" = pure running time at max speed (distance/speed)
            travelTimeWithoutDwellTimesNoCurvesSeconds: noDwellNoCurvesTimeSeconds,
            // Recalculate speeds based on curve-aware times
            averageSpeedWithoutDwellTimesMetersPerSecond:
                analysis.totalNoDwellCurveTimeSeconds > 0
                    ...
                    : 0,
            // Speed without curves = max running speed
            averageSpeedWithoutDwellTimesNoCurvesMetersPerSecond: runningSpeedMps,
            operatingSpeedMetersPerSecond:
                analysis.totalTimeWithCurvesSeconds + totalDwellTimeSeconds > 0
                    ? analysis.totalDistanceMeters / (analysis.totalTimeWithCurvesSeconds + totalDwellTimeSeconds)
                    : 0,
            geometryResolution: analysis.geometryResolution
        };
    }, [path, pathMode, pathData.totalDwellTimeSeconds, accelerationMps2, decelerationMps2, maxSpeedKmH]);

    // Use curve-aware stats for rail modes, otherwise use stored values
