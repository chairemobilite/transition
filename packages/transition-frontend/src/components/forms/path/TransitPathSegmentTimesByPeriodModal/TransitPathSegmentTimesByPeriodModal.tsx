...
const TransitPathSegmentTimesByPeriodModal: React.FunctionComponent<TransitPathSegmentTimesByPeriodModalProps> = (
    ...
    return (
        ...
        >
            ...
            <h3 style={{ marginBottom: '0.5rem', flexShrink: 0 }}>
                {t('transit:transitPath:SegmentTimesByPeriod')}
            </h3>
            ...
            <SegmentTimesToolbar
                ...
                selectedPeriodsGroup={selectedPeriodsGroup}
                periodsGroupChoices={periodsGroupChoices}
                onPeriodsGroupChange={setSelectedPeriodsGroup}
            />
            ...
            >
                    ...
                    <>
                        ...
                        >
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                <div style={{ width: 20, height: 20, borderRadius: '50%', background: '#4fc3f7', border: '3px solid #fff', flexShrink: 0 }} />
                                <div style={{ flex: 1, height: 4, background: '#4fc3f7', borderRadius: 2, position: 'relative' }}>
                                    <span style={{ position: 'absolute', top: '-1.3em', left: '50%', transform: 'translateX(-50%)', fontSize: '0.75em', whiteSpace: 'nowrap', color: 'rgba(255,255,255,0.6)' }}>
                ...
                {editMode === 'checkpoint' && activeCheckpoint && (
                    <>
                        ...
                        >
                            <span style={{ position: 'absolute', top: '-1.3em', left: '50%', transform: 'translateX(-50%)', fontSize: '0.75em', whiteSpace: 'nowrap', color: 'rgba(255,255,255,0.6)' }}>
                                {t('transit:transitPath:Checkpoint')} {activeCheckpointIndex + 1}/{checkpoints.length}
                                {' '}({activeCheckpoint.toNodeIndex - activeCheckpoint.fromNodeIndex} {t('transit:transitPath:Segments').toLowerCase()})
                            </span>
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                <div style={{ width: 20, height: 20, borderRadius: '50%', background: '#ff9800', border: '3px solid #fff', flexShrink: 0 }} />
                                {Array.from(
                                    { length: Math.min(activeCheckpoint.toNodeIndex - activeCheckpoint.fromNodeIndex - 1, 30) },
                                    (_, i) => (
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.5rem', flexShrink: 0, justifyContent: 'flex-end' }}>
                            <Button
                                color="blue"
                        ...
                        <CheckpointPeriodTimesTable
                            ...
                            periods={periods}
                            language={i18n.language}
                            getCurrentTotal={(periodShortname) => getCheckpointCurrentTotal(activeCheckpoint, periodShortname)}
                            getTarget={(periodShortname) => getCheckpointTarget(activeCheckpoint, periodShortname)}
                            onTargetChange={(periodShortname, newSec) => setCheckpointTarget(activeCheckpoint, periodShortname, newSec)}
                        />
                    </>
                )}
            </div>
