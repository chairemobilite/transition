...
import { UserDefinedConfigEditComponent } from '../../userDefinedConfig/OptionsDescriptorWidgets';
        ...
        <React.Fragment>
            ...
            {algoDescriptor && (
                <UserDefinedConfigEditComponent
                    key={`algorithmConfigOptions${props.algorithmConfig?.type ?? ''}`} // Force remount when type changes to reset internal state
                    value={props.algorithmConfig?.config ?? {}}
                    configSchema={algoDescriptor}
                    disabled={props.disabled}
