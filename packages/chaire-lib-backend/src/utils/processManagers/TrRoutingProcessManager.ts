...
const startTrRoutingProcess = async (
    ...
    const processStatus = await ProcessManager.startProcess({
        ...
        commandArgs,
        waitString,
        useShell: true, // For unknown reason yet, libmemcached pool need trRouting to be run in a shell to work
        cwd,
        attemptRestart,
        logFiles: {
...
const startBatch = async function (
    ...
    // Start Memcached
    const startMemcached = true; // TODO implement a config option to allow disable this
    let memcachedServer;
    if (startMemcached) {
