...
describe('OSRM Mode tests', function() {

    /********************************/
    /* Constructor validation tests */
    
    test('Create valid Mode', async () => {
        let aMode = new OSRMMode('Walking', 'localhost', 4000);
    ...
    test('Test basic fetch', async () => {
        ...
        let result = await aMode.route(params);
        expect(fetchMock).toHaveBeenCalledTimes(1);
        expect(fetchMock).toHaveBeenCalledWith('http://localhost:4000/route/v1/walking/-73,45;-73.1,45.1?alternatives=false&steps=false&annotations=false&continue_straight=default&geometries=geojson&overview=full', {});
    ...
    test('Test simple tableFrom call', async () => {
        ...
        let aMode = new OSRMMode('walking', '', 5000);

        const origin = TestUtils.makePoint([-73.663473,45.611544]);
        const destinations = [TestUtils.makePoint([-73.663473,45.611544]),
        ...
        expect(fetchMock).toHaveBeenCalledTimes(1);
        expect(fetchMock).toHaveBeenCalledWith(EXPECTED_QUERY, {});
