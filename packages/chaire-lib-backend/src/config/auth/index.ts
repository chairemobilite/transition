                ...
                knex.select(
                    ...
                    .then(async (rows) => {
                        if (rows.length === 1) {
                            ...
                            const newUser = await saveNewUser({ googleId: profile.id, isTest: false });
                ...
                knex.select(
                    ...
                    .then(async (rows) => {
                        if (rows.length === 1) {
                            ...
                            const newUser = await saveNewUser({ facebookId: profile.id, isTest: false });
                            done(null, newUser !== null ? newUser.sanitize() : false);
                        }
                        return null;
                    })
