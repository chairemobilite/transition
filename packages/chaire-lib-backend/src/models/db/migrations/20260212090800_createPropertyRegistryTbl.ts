...
export async function up(knex: Knex): Promise<unknown> {
    ...
    await knex.schema.createTable(tableName, (table: Knex.TableBuilder) => {
        // Primary key (integer for efficient indexing on large datasets)
        ...
        table.specificType('building_polygon', 'geography(MULTIPOLYGON)');
        table.specificType('parcel_polygon', 'geography(MULTIPOLYGON)');
        table.specificType('building_centroid_point', 'geography(POINT)');
        table.specificType('parcel_centroid_point', 'geography(POINT)');
        table.specificType('main_entrance_point', 'geography(POINT)');
    ...
    await knex.raw(`CREATE INDEX ${tableName}_building_polygon_idx ON ${tableName} USING gist (building_polygon)`);
    await knex.raw(`CREATE INDEX ${tableName}_parcel_polygon_idx ON ${tableName} USING gist (parcel_polygon)`);
    await knex.raw(
        `CREATE INDEX ${tableName}_building_centroid_point_idx ON ${tableName} USING gist (building_centroid_point)`
    );
    await knex.raw(
        `CREATE INDEX ${tableName}_parcel_centroid_point_idx ON ${tableName} USING gist (parcel_centroid_point)`
    );
    await knex.raw(
        `CREATE INDEX ${tableName}_main_entrance_point_idx ON ${tableName} USING gist (main_entrance_point)`
    );
