...
import { TerraDraw, TerraDrawSelectMode, TerraDrawPolygonMode, GeoJSONStoreFeatures } from 'terra-draw';
...
/** TerraDraw event names - 'finish' is emitted when a polygon drawing is completed */
type TerraDrawEvent = 'change' | 'finish';

/**
 * Creates an empty polygon feature used as a sentinel value when clearing selections.
 * Used to signal deletion to callbacks without having actual polygon data.
 */
const createEmptyPolygonFeature = (): GeoJson.Feature<GeoJson.Polygon> => ({
    type: 'Feature',
    properties: {},
    geometry: { type: 'Polygon', coordinates: [] }
});

// Define the wrapper type to match what TransitionMainMap expects
export type TerraDrawAdapter = {
    getAll: () => GeoJson.FeatureCollection;
    deleteAll: () => TerraDrawAdapter;
    add: (geojson: GeoJson.Feature<GeoJson.Polygon> | GeoJson.FeatureCollection<GeoJson.Polygon>) => string[];
    reset: () => void;
    onRemove: () => void;
};
...
class TerraDrawControl implements IControl {
    ...
    _polygonRoot: ReturnType<typeof createRoot> | undefined;
    _trashRoot: ReturnType<typeof createRoot> | undefined;
    ...
    deleteAll() {
        try {
            ...
            this._deleteCallback(createEmptyPolygonFeature());
    ...
    onAdd(map: MapLibreGLMap): HTMLElement {
        ...
        this._polygonRoot = createRoot(this._polygonButton);
        this._polygonRoot.render(
            React.createElement(FontAwesomeIcon, { icon: faDrawPolygon, style: { fontSize: '16px' } })
        );
        ...
        this._trashRoot = createRoot(this._trashButton);
        this._trashRoot.render(React.createElement(FontAwesomeIcon, { icon: faTrashCan, style: { fontSize: '16px' } }));
    ...
    onRemove() {
        ...
        this._polygonRoot?.unmount();
        this._polygonRoot = undefined;
        this._trashRoot?.unmount();
        this._trashRoot = undefined;
...
const getTerraDraw = (
    ...
    try {
        ...
        // If start fails, cleanup and abort initialization to avoid registering
        // handlers/controls against a failed TerraDraw instance
        cleanUpTerraDrawSources(map);
        console.error('TerraDraw failed to start');
        return undefined;
    }
    ...
    draw.on('change', () => {
        ...
        if (feature) {
            ...
            if (snapshot.length === 0) {
                deleteCallback(createEmptyPolygonFeature());
                updateControl();
            }
        }
    });
    ...
    draw.on('finish' as TerraDrawEvent, () => {
        draw.setMode('polygon');
        // Notify external listeners of mode change
        modeChangeCallback({ mode: 'draw_polygon' });
    ...
    const drawWrapper: TerraDrawAdapter = {
        ...
        add: (geojson) => {
            ...
            if (geojson.type === 'FeatureCollection') {
                draw.addFeatures(geojson.features as GeoJSONStoreFeatures[]);
                return geojson.features.map((f) => (f.id as string) || '');
            } else {
                draw.addFeatures([geojson] as GeoJSONStoreFeatures[]);
