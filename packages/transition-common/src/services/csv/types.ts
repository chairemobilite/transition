...
export type CsvFieldMappingDescriptor = {
     ...
     * * `time` means the mapping represents a time, so for each, there should
     *   be an additional question about the type of time (departure/arrival)
     *   and the format (), that will be stored in keys `${key}Type` and
     *   `${key}Format`. The labels for the additional type and format questions
     *   is the `i18nLabel` value with `Type` and `Format` (HH:MM, HMM,
     *   secondsSinceMidnight) appended.
