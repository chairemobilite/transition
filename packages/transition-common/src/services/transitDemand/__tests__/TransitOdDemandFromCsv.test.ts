...
import { BatchRoutingOdDemandFromCsvAttributes } from '../types';
...
const defaultCsvFileAndMapping: BatchRoutingOdDemandFromCsvAttributes = {
    type: 'csv',
    fileAndMapping: {
    ...
    csvFields: ['trip_id', 'origin_latitude', 'origin_longitude', 'dest_latitude', 'dest_longitude', 'origLat', 'origLon', 'destLat', 'destLon', 'trip_time']
...
describe('TransitOdDemandFromCsv', () => {
    ...
    describe('Transit-Specific Validation', () => {
        ...
        beforeEach(() => {
            // Start with an uninitialized mapping
            const notInitialized = _cloneDeep(defaultCsvFileAndMapping)
            notInitialized.fileAndMapping.fieldMappings = {} as any;
            demand = new TransitOdDemandFromCsv(_cloneDeep(notInitialized));
        });
        ...
        test('should validate complete transit demand mapping', () => {
            ...
            expect(demand.getPartialFieldMappings()).toEqual({
                id: 'trip_id',
                projection: '4326',
