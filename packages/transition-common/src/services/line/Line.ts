...
export interface LineAttributes extends GenericAttributes {
    ...
    data: {
        gtfs?: GtfsRoute;
        /**
         * Travel time in seconds to move an empty vehicle from the end of one
         * path to the start of another (deadhead / hors-service). Nested by
         * from-path id, then to-path id. `null` when routing found no duration.
         */
            ...
            for (const path2 of completePaths) {
                ...
                if (routingEngine === 'engine') {
                    ...
                    if (path2TerminalNodeId === path1TerminalNodeId) {
                        ...
                        if (travelTimeSeconds === null || travelTimeSeconds === undefined || travelTimeSeconds < 0) {
                            console.error(
