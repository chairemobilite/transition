...
test('Generate From Routing throws on non-finite duration (non-routed engine, zero running speed)', () => {
    const simplePath = new TransitPathStub({
        id: 'path1',
        line_id: line.get('id'),
        nodes: [ node1.properties.id, node4.properties.id ],
    ...
    const nodeGeojson = PathGeographyUtils.prepareNodesAndWaypoints(simplePath, DEFAULT_SPEED / 3.6);
    expect(() => generatePathGeographyFromRouting(simplePath, nodeGeojson, [validRoutingResult])).toThrow(TrError);
});
...
describe('Node insert and remove operations', () => {
    ...
    test('insert node at beginning', async () => {
        ...
        expect(path.attributes.data.segments[0].travelTimeSeconds).toEqual(
            Math.ceil(storedSegmentDuration(legsAfterInsert[0].distance, legsAfterInsert[0].duration) * ratio)
        );
    });
    ...
    test('insert node at end', async () => {
        ...
        expect(path.attributes.data.segments[3].travelTimeSeconds).toEqual(
            Math.ceil(storedSegmentDuration(legsAfterInsert[3].distance, legsAfterInsert[3].duration) * ratio)
        );
    });
...
describe('Waypoint change operations', () => {
    ...
    test('recalculates affected segment only', async() => {
        ...
        expect(path.attributes.data.segments[0].travelTimeSeconds).toEqual(Math.ceil(storedSegmentDuration(changedSegDistance, changedSegDuration) * ratio));
    });
