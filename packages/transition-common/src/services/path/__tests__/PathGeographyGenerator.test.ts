...
const storedSegmentDuration = (distanceMeters: number, routedDurationSeconds: number, ratio: number = 1) =>
    ceilToPositiveInteger(segmentDuration(distanceMeters, routedDurationSeconds) * ratio);
...
describe('Bad input data that will throw on generation', () => {
    ...
    test.each([
        {
            title: 'Generate From Routing throws on zero OSRM duration',
            routingResults: {
                tracepoints: [node1, node4],
                matchings: [{
                    confidence: 99,
            ...
            title: 'Generate From Routing throws on NaN physics duration (zero-distance segment)',
            ...
            title: 'Generate From Routing throws on non-finite duration (non-routed engine, zero running speed)',
            // routingEngine is not 'engine' and defaultRunningSpeedKmH is 0 (not blank), so
            // runningSpeedMps is 0 and the physics model returns Infinity (distance > 0), which
            // isNaN would miss but !isFinite catches
            routingResults: {
                tracepoints: [node1, node4],
                matchings: [{
                    confidence: 99,
            ...
            defaultRunningSpeed: 0,
            engine: 'engineCustom'
        }, {
            title: 'Generate From Routing throws on negative routing duration',
            // duration is somehow negative
            routingResults: {
                tracepoints: [node1, node4],
                matchings: [{
                    confidence: 99,
    ...
    ])('$title', ({ routingResults, engine = 'engine', defaultRunningSpeed = DEFAULT_SPEED }) => {
        const simplePath = new TransitPathStub({
            id: 'path1',
            line_id: line.get('id'),
            nodes: [ node1.properties.id, node4.properties.id ],
            data: {
                ...
                defaultAcceleration: DEFAULT_ACC_DEC,
                defaultDeceleration: DEFAULT_ACC_DEC,
                defaultRunningSpeedKmH: defaultRunningSpeed,
                maxRunningSpeedKmH: DEFAULT_MAX_SPEED
            }
        }) as any;

        const nodeGeojson = PathGeographyUtils.prepareNodesAndWaypoints(simplePath, DEFAULT_SPEED / 3.6);
        expect(() => generatePathGeographyFromRouting(simplePath, nodeGeojson, [routingResults])).toThrow(TrError);
    });
...
describe('Node insert and remove operations', () => {
    ...
    test('insert node at beginning', async () => {
        ...
        expect(path.attributes.data.segments[0].travelTimeSeconds).toEqual(
            storedSegmentDuration(legsAfterInsert[0].distance, legsAfterInsert[0].duration, ratio)
        );
    });
    ...
    test('insert node in middle', async () => {
        ...
        expect(path.attributes.data.segments[1].travelTimeSeconds).toEqual(
            Math.ceil(storedSegmentDuration(legsAfterInsert[1].distance, legsAfterInsert[1].duration) * ratio)
        );
        expect(path.attributes.data.segments[2].travelTimeSeconds).toEqual(
            Math.ceil(storedSegmentDuration(legsAfterInsert[2].distance, legsAfterInsert[2].duration) * ratio)
        );
    });
    ...
    test('insert node at end', async () => {
        ...
        expect(path.attributes.data.segments[3].travelTimeSeconds).toEqual(
            storedSegmentDuration(legsAfterInsert[3].distance, legsAfterInsert[3].duration, ratio)
        );
    });
    ...
    test('remove middle node', async () => {
        ...
        expect(path.attributes.data.segments[0].travelTimeSeconds).toEqual(
            Math.ceil(storedSegmentDuration(legsAfterRemove[0].distance, legsAfterRemove[0].duration) * ratio)
        );
    });
...
describe('Custom dwell time preservation across node changes', () => {
    ...
    test('preserved custom dwells survive an insert at end, while the newly inserted node uses its node default', () => {
        // Previous: [node1, node4, node6] with custom dwells [0, 42, 55] (differ from node defaults).
        // Insert node3 at end → [node1, node4, node6, node3] (seg 0 preserved, seg 1 preserved, seg 2 new).
        const path = makePathWithCustomDwellsAndInsertAtEnd([0, 42, 55]);

        // First node: always 0
        expect(path.attributes.data.dwellTimeSeconds[0]).toEqual(0);
        // Segment 1 (node4→node6): node4 preserved with custom 42, not 120 (NODE4_DWELL_TIME default)
        expect(path.attributes.data.dwellTimeSeconds[1]).toEqual(42);
        // Segment 2 (node6→node3) is NEW but starts at node6 (preserved) → keep custom 55, not node6's default
        expect(path.attributes.data.dwellTimeSeconds[2]).toEqual(55);
        // Last node (node3) is newly inserted, no stored dwell → falls back to node default
...
describe('Waypoint change operations', () => {
    ...
    test('recalculates affected segment only', async() => {
        ...
        expect(path.attributes.data.segments[0].travelTimeSeconds).toEqual(storedSegmentDuration(changedSegDistance, changedSegDuration, ratio));
    });
...
describe('segmentsByServiceAndPeriod remapping on path edits', () => {
    ...
    test('insert node in middle preserves unchanged period segments and scales new ones', async () => {
        ...
        expect(amData.segments[1].travelTimeSeconds).toEqual(ceilToPositiveInteger(segmentDuration(legs[1].distance, legs[1].duration) * amRatio));
        expect(amData.segments[2].travelTimeSeconds).toEqual(ceilToPositiveInteger(segmentDuration(legs[2].distance, legs[2].duration) * amRatio));
        ...
        expect(offData.segments[1].travelTimeSeconds).toEqual(ceilToPositiveInteger(segmentDuration(legs[1].distance, legs[1].duration) * offRatio));
        expect(offData.segments[2].travelTimeSeconds).toEqual(ceilToPositiveInteger(segmentDuration(legs[2].distance, legs[2].duration) * offRatio));
    ...
    test('insert node at beginning shifts period segments correctly', async () => {
        ...
        expect(amData.segments[0].travelTimeSeconds).toEqual(ceilToPositiveInteger(segmentDuration(legs[0].distance, legs[0].duration) * amRatio));
        ...
        expect(offData.segments[0].travelTimeSeconds).toEqual(ceilToPositiveInteger(segmentDuration(legs[0].distance, legs[0].duration) * offRatio));
    ...
    test('insert node at end appends new period segment', async () => {
        ...
        expect(amData.segments[3].travelTimeSeconds).toEqual(ceilToPositiveInteger(segmentDuration(legs[3].distance, legs[3].duration) * amRatio));
        ...
        expect(offData.segments[3].travelTimeSeconds).toEqual(ceilToPositiveInteger(segmentDuration(legs[3].distance, legs[3].duration) * offRatio));
    ...
    test('multiple services within a period each updated independently', async () => {
        ...
        expect(amService1.segments[3].travelTimeSeconds).toEqual(ceilToPositiveInteger(segmentDuration(legs[3].distance, legs[3].duration) * s1Ratio));
        ...
        expect(amService2.segments[3].travelTimeSeconds).toEqual(ceilToPositiveInteger(segmentDuration(legs[3].distance, legs[3].duration) * s2Ratio));
    ...
    test('waypoint change period data for affected segment recalculated', async () => {
        ...
        const changedSegDuration = legs[0].duration + legs[1].duration;
        expect(amData.segments[0].travelTimeSeconds).toEqual(ceilToPositiveInteger(segmentDuration(changedSegDistance, changedSegDuration) * amRatio));
    ...
    test('all segments new defaults period ratio to 1.0', async () => {
        ...
        expect(amData.segments[0].travelTimeSeconds).toEqual(ceilToPositiveInteger(segmentDuration(legs[0].distance, legs[0].duration) * 1.0));
        expect(amData.segments[1].travelTimeSeconds).toEqual(ceilToPositiveInteger(segmentDuration(legs[1].distance, legs[1].duration) * 1.0));
