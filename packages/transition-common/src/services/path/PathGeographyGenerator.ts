 ...
 * @returns The remapped travel time for each segment, as integers
 */
const computeSegmentTravelTimes = (
    ...
    return initialSegmentInfos.map((segment, i) =>
        segment.isNew
            ? ceilToPositiveInteger(durationsFromRouting[i] * ratio)
            : segment.initialTravelTimeSeconds - dwellAdjustmentsSeconds[i]
    );
};
 ...
 * @returns Segment geometry, per-segment time/distance data, and dwell times, not rounded
 */
const buildSegmentsAndGeometry = (
    ...
    for (let i = 0; i < routing.legs.length; i++) {
        ...
        if (i === routing.legs.length - 1 && !nodeIds[nextNodeIndex]) {
            ...
            segmentsData.push({
                travelTimeSeconds: routedDurationSeconds,
 ...
 * @param current - Current routing results, without rounding (segments, durations, dwell times)
