...
const dayAbbreviations: Record<string, Record<string, string>> = {
    monday: { fr: 'Lun', en: 'Mon' },
    tuesday: { fr: 'Mar', en: 'Tue' },
    wednesday: { fr: 'Mer', en: 'Wed' },
    thursday: { fr: 'Jeu', en: 'Thu' },
    friday: { fr: 'Ven', en: 'Fri' },
    saturday: { fr: 'Sam', en: 'Sat' },
    sunday: { fr: 'Dim', en: 'Sun' }
};
 ...
 * Examples: "Every day", "Weekday", "Weekend", "Sat", "Mon-Wed-Fri", "Holiday".
 * When a commonName is provided, it is appended to the label so that groups
 * sharing the same day pattern (e.g. several seasonal weekday schedules) can
 * be distinguished by a shared token from their service names.
 *
 * @param activeDays - Days of the week covered by the group (e.g. ['monday', 'tuesday', ...])
 * @param hasHolidayService - Whether the group contains holiday services
 * @param language - Language code for the label ('fr' or 'en')
 * @param commonName - Optional shared token extracted from the group's service names
 */
export const buildGroupLabel = (
    activeDays: string[],
    hasHolidayService: boolean,
    language: string,
    commonName?: string
    ...
    const holidayLabel = language === 'fr' ? 'Jour férié' : 'Holiday';
    const weekdayLabel = language === 'fr' ? 'Semaine' : 'Weekday';
    const weekendLabel = language === 'fr' ? 'Fin de semaine' : 'Weekend';
    const everyDayLabel = language === 'fr' ? 'Tous les jours' : 'Every day';

    const parts: string[] = [];

    if (hasAllWeekdays && hasWeekend) {
        parts.push(everyDayLabel);
    } else if (hasAllWeekdays) {
        parts.push(weekdayLabel);
    } else if (hasWeekend) {
        parts.push(weekendLabel);
    } else if (hasSaturday) {
        parts.push(dayAbbreviations.saturday[language]);
    } else if (hasSunday) {
        parts.push(dayAbbreviations.sunday[language]);
    }

    // Individual weekdays not covered by "Weekday"
    if (!hasAllWeekdays) {
        const individualDays = weekdays.filter((d) => daysSet.has(d)).map((d) => dayAbbreviations[d][language]);
    ...
    if (hasHolidayService) {
        if (parts.length > 0) {
            parts[parts.length - 1] = `${parts[parts.length - 1]}, ${holidayLabel}`;
        } else {
            parts.push(holidayLabel);
        }
    }
