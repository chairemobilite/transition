...
import { BatchRoutingOdDemandFromCsvAttributes } from '../transitDemand/types';

export class TransitBatchRoutingCalculator {
    private static async _calculate(
        params: BatchRoutingOdDemandFromCsvAttributes,
    ...
    static async calculate(
        ...
        if (!parameters) {
            throw new TrError(
                'cannot calculate transit batch route: demand parameters not set',
                'TRBROUTING0003',
                'transit:transitRouting:errors:TransitBatchRouteCannotBeCalculatedBecauseError'
            );
        }
