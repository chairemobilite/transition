 ...
 * 'boolean' | 'nested' | 'select' | 'multiselect' | 'csvFile')}
 ...
 * one or multiple values from a list of choices.
...
export type UserDefinedConfigFieldDescriptor = UserDefinedConfigFieldBase &
    (UserDefinedConfigFieldByType | NestedFieldDescriptor<any>);

/**
 * User defined config schema. This class describes the name and options
...
export function validateFieldsWithSchema<T extends Record<string, unknown>>(
    ...
    for (const [key, fieldDef] of Object.entries(fieldDefinitions)) {
        ...
        if (fieldDef.required && (value === undefined || value === null || value === '')) {
            valid = false;
            errors.push(fieldDef.i18nName + 'Required');
            continue;
        }
        if (fieldDef.type === 'nested') {
            // Handle nested options recursively
            ...
            if ('validate' in fieldDef && fieldDef.validate) {
            ...
            } else if (fieldDef.type === 'percentage') {
                ...
                if (numberValue < 0 || numberValue > 1) {
                    valid = false;
                    errors.push(fieldDef.i18nName + 'Invalid');
                }
