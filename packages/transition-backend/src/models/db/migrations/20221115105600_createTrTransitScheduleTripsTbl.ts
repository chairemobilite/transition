...
export async function up(knex: Knex): Promise<unknown> {
    ...
    await knex.schema.createTable(tableName, (table: Knex.TableBuilder) => {
        ...
        table
            .foreign('schedule_period_id')
            .references('tr_transit_schedule_periods.id')
            .onDelete('CASCADE');
        table
            .uuid('path_id')
            .notNullable()
        ...
        table
            .foreign('unit_id')
            .references('tr_transit_units.id')
            .onDelete('CASCADE');
        table.uuid('block_id').index(); // optional
        table
            .integer('departure_time_seconds')
            .notNullable()
            .index(); // required
        table
            .integer('arrival_time_seconds')
            .notNullable()
            .index(); // required
