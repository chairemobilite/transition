...
export async function up(knex: Knex): Promise<unknown> {
    ...
    await knex.schema.createTable(tableName, (table: Knex.TableBuilder) => {
        ...
        table.uuid('service_id').notNullable().index(); // required
        table.foreign('service_id').references('tr_transit_services.id').onDelete('CASCADE');
        table.string('periods_group_shortname').notNullable().index(); // required, for schedules with no associated periods, choose all_day as periods group shortname and all_day as period shortname
        table.boolean('allow_seconds_based_schedules').defaultTo(false).index();
