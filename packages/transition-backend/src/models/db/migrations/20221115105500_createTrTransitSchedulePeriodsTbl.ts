...
export async function up(knex: Knex): Promise<unknown> {
    ...
    await knex.schema.createTable(tableName, (table: Knex.TableBuilder) => {
        ...
        table.string('period_shortname').notNullable().index(); // required
        ...
        table.integer('period_start_at_seconds').notNullable().index(); // required
        table.integer('period_end_at_seconds').notNullable().index(); // required
