...
/**
 * Prepare POI data for bulk insert into temporary table
 ...
 * @returns Object with placeholders and bindings for SQL insert
 */
function preparePOIData(
    poisFeatureCollection: GeoJSON.FeatureCollection<GeoJSON.Point, { weight?: number }>,
    errorCodePrefix: string
): { placeholders: string[]; bindings: (string | number)[] } {
    ...
    return { placeholders: poiValuePlaceholders, bindings: poiBindings };
}
 ...
 * @param poiData Prepared POI data with placeholders and bindings
...
async function createAndPopulatePOIsTable(
    ...
    uniqueId: string,
    poiData: { placeholders: string[]; bindings: (string | number)[] },
    ...
    await trx.raw(
        ...
        VALUES ${poiData.placeholders.join(', ')}
        `,
        [tempPoisTableName, idColumnName, ...poiData.bindings]
    );
...
export async function getPOIsWithinBirdDistanceFromPlaces(
    ...
    try {
        return await knex.transaction(async (trx) => {
            ...
            // Prepare POI data using common helper
            const poiData = preparePOIData(poisFeatureCollection, 'DBQPOIBDP');
            ...
            const tempPoisTableName = await createAndPopulatePOIsTable(trx, uniqueId, poiData);
...
export async function getPOIsWithinBirdDistanceFromNodes(
    ...
    try {
        return await knex.transaction(async (trx) => {
            ...
            // Prepare POI data using common helper
            const poiData = preparePOIData(poisFeatureCollection, 'DBQPOIBDN');
            ...
            const hasNodeIds = nodeIds !== undefined && nodeIds.length > 0;
            const queryResult = await trx.raw(
                hasNodeIds
                    ? `
                SELECT
                    ...
                    p.poi_id,
                    p.weight,
                    ST_Distance(n.geography, p.geography) as distance,
                    ST_AsGeoJSON(p.geography)::jsonb as geography
                FROM tr_transit_nodes n
                CROSS JOIN ?? p
                WHERE n.id = ANY(?::uuid[])
                    AND n.is_enabled IS TRUE
                    AND ST_DWithin(n.geography, p.geography, ?)
                ...
                ORDER BY n.id, distance
                `,
                hasNodeIds ? [tempPoisTableName, nodeIds, distanceMeters] : [tempPoisTableName, distanceMeters]
            );

            // Group results by node_id using common helper
            return groupResultsByEntityId(queryResult.rows, 'node_id');
        });
    } catch (error) {
        if (error instanceof TrError) {
            throw error;
...
export async function getPOIsWithinBirdDistanceFromPoint(
    ...
    try {
        return await knex.transaction(async (trx) => {
            ...
            // Prepare POI data using common helper
            const poiData = preparePOIData(poisFeatureCollection, 'DBQPOIBD');

            // Create and populate POIs table using common helper (using 'id' column name)
            const tempTableName = await createAndPopulatePOIsTable(trx, uniqueId, poiData, 'id');
