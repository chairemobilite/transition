...
describe('clipPolygon', () => {
    ...
    test('should handle many circles efficiently', async () => {
        ...
        // Should complete in reasonable time (< 5 seconds for 50 circles)
    ...
    test('should handle transaction properly and clean up temp tables', async () => {
        ...
        // Should be 0 or very low if other tests are running in parallel
        expect(parseInt(tempTables.rows[0].count)).toBeLessThan(10);
    });

    test('should throw error when database connection fails', async () => {
        // This test ensures error handling works
        // We can't easily simulate DB failure, but we can test the error path exists
    ...
    test('should verify quad_segs parameter produces expected point count', async () => {
        const circles = [
            { center: [-73.5, 45.5] as [number, number], radiusKm: 0.5 }
        ];
        ...
        // With quad_segs=6, each circle should have exactly 25 points
        // (6 segments per quarter * 4 quarters = 24 segments + 1 closure point)
        expect(result[0][0].length).toBe(25);
    });
