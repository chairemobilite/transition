...
const getPOIsCategoriesCountInPolygon = async (
    accessibilityPolygon: GeoJSON.MultiPolygon
): Promise<{
    ...
    try {
        ...
        for (const row of categoriesResponse.rows) {
            accessiblePlacesCountByCategory[row.category] = row.count;
        }
        ...
        for (const row of detailedCategoriesResponse.rows) {
            accessiblePlacesCountByDetailedCategory[row.category_detailed] = row.count;
        }
