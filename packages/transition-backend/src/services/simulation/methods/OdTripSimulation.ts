...
export default class OdTripSimulation implements SimulationMethod {
    ...
    private async sampleOdTripFile(routingJob: ExecutableJob<BatchRouteJobType>): Promise<void> {
    ...
    async simulate(
        ...
        const demandParameters = _cloneDeep(this.options.demandAttributes);
        demandParameters.fileAndMapping.fieldMappings.time = timeCsvColumnHeader;
        demandParameters.fileAndMapping.fieldMappings.timeFormat = 'secondsSinceMidnight';
        demandParameters.fileAndMapping.fieldMappings.timeType = 'departure';
        if (!demandParameters.csvFields.includes(timeCsvColumnHeader)){
            demandParameters.csvFields.push(timeCsvColumnHeader);
        }
        ...
        await this.sampleOdTripFile(routingJob);
