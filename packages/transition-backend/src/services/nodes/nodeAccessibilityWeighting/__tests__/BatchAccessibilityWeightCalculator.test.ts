...
const makePoint = (lon: number, lat: number, intrinsicWeight?: number): IntrinsicWeightedPoint => ({
    type: 'Feature',
    geometry: { type: 'Point', coordinates: [lon, lat] },
...
const nodeFeatures: NodeFeatureForWeighting[] = [
    ...
    makeNodeFeature('node-c', -73.51, 45.51)
];
    ...
    return {
        routingService: {
            tableManyToMany: jest.fn().mockResolvedValue({
                query: '',
                durations: overrides?.manyToManyResult?.durations ?? [
                    [180, 420],
                    [200, 350]
                ],
                distances: overrides?.manyToManyResult?.distances ?? [
                    [200, 500],
                    [250, 400]
                ]
            }),
        ...
        getNodesInBirdDistanceFromPoint: jest
            .fn()
            .mockImplementation((_point: GeoJSON.Point) =>
                Promise.resolve(overrides?.candidateNodesFn ? overrides.candidateNodesFn(_point) : defaultCandidates)
            )
    };
}
...
describe('BatchAccessibilityWeightCalculator', () => {
    describe('calculateBatchAccessibilityWeights', () => {
        test('returns empty map when no points are provided', async () => {
            ...
            const result = await calculateBatchAccessibilityWeights([], nodeFeatures, defaultParameters, deps);
        ...
        test('returns empty map when no candidates found', async () => {
            ...
            const result = await calculateBatchAccessibilityWeights(points, nodeFeatures, defaultParameters, deps);
            expect(result.size).toBe(0);
        });
        ...
        test('respects intrinsic weight from point properties', async () => {
            ...
            const result1 = await calculateBatchAccessibilityWeights([point1], nodeFeatures, defaultParameters, deps1);
            const result2 = await calculateBatchAccessibilityWeights([point2], nodeFeatures, defaultParameters, deps2);
        ...
        test('calls onProgress callback per batch', async () => {
            const points = Array.from({ length: 5 }, (_, i) => makePoint(-73.5 + i * 0.001, 45.5));
            const deps = makeMockDeps({
                manyToManyResult: {
            ...
            await calculateBatchAccessibilityWeights(points, nodeFeatures, defaultParameters, deps, {
                batchSize: 3,
                concurrency: 1,
        ...
        test('throws when source count exceeds maxPairs', async () => {
            const points = [makePoint(-73.5, 45.5), makePoint(-73.501, 45.501)];
            const deps = makeMockDeps();

            await expect(
                calculateBatchAccessibilityWeights(points, nodeFeatures, defaultParameters, deps, {
                    batchSize: 5,
                    maxOsrmTablePairs: 1
                })
...
describe('getBirdDistanceRadiusMeters', () => {
    it.each([
        ...
        [
            DEFAULT_MAX_WALKING_TIME_SECONDS,
            DEFAULT_WALKING_SPEED_MPS,
            DEFAULT_MAX_WALKING_TIME_SECONDS * DEFAULT_WALKING_SPEED_MPS
        ]
    ])('for maxWalkingTimeSeconds=%d and walkingSpeedMps=%f, returns %f meters', (maxTime, speed, expected) => {
...
describe('getIntrinsicWeight', () => {
    ...
    ])('returns the property value when %s is provided', (_label, weight, expected) => {
    ...
    ])('returns DEFAULT_INTRINSIC_WEIGHT when weight is %s', (_label, weight) => {
