...
export type BatchWeightingOptions = {
    ...
    /**
     * Called before each batch. Return false to stop processing early
     * (e.g. for cancellation or pause-then-cancel). When the returned
     * promise resolves to false, the loop exits and the partial results
        ...
        queue.add(async () => {
            ...
            try {
                await processBatch(
                    ...
                    parameters,
                    dependencies,
                    nodeAccessibilityWeights
                );
                processedPoints += batch.length;
                options.onProgress?.(processedPoints, points.length);
            } catch (error) {
