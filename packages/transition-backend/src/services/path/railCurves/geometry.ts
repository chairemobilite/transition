...
import type { Feature, Point, Position as GeoPosition } from 'geojson';
import type { RailMode } from 'transition-common/lib/services/line/types';
import type { TurnDirection, CurveRadiusOptions } from 'transition-common/lib/services/path/railCurves/types';
import {
    resolveOptions,
...
/**
 * Identifies vertices where the geometry is too coarse for reliable curve
 * analysis: deflection angle >= MIN_DEFLECTION_ANGLE_RAD AND local spacing
 * >= MIN_COARSE_VERTEX_SPACING_METERS.
 *
 * @param coordinates Array of [lon, lat] positions
 * @returns GeoJSON Point features for each flagged vertex
 */
export function computeLargeAngleVertices(coordinates: GeoPosition[]): Feature<Point>[] {
    const features: Feature<Point>[] = [];
    if (coordinates.length < 3) return features;
