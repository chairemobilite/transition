...
export type StopTimeDistances = {
    distanceTraveled: number;
    distanceFromShape: number;
};

export type SegmentTimeResults = {
 ...
 */
export const calculateLayoverTimeSeconds = (
 ...
 */
export const cleanSegmentCoordinates = (segmentGeojson: GeoJSON.Feature<GeoJSON.LineString>): number[][] => {
 ...
 */
export const normalizeDistancesToMeters = (
 ...
 * @returns Merged coordinates, segment start indices, and per-segment distances in meters
 */
export const sliceShapeIntoSegments = (
    completeShape: GeoJSON.Feature<GeoJSON.LineString>,
    stopTimeDistances: StopTimeDistances[],
    stopTimesCount: number
): { globalCoordinates: number[][]; segments: number[]; segmentDistancesMeters: number[] } => {
    let globalCoordinates: number[][] = [];
    ...
    return { globalCoordinates, segments, segmentDistancesMeters };
};
 ...
 */
export const buildPathData = (params: {
...
export const generateGeographyAndSegmentsFromGtfs = (
    ...
    if (shapeDistancesFailed) {
        ...
        const segmentTimes = computeSegmentTimesFromStopTimes(stopTimes, sliceResult.segmentDistancesMeters);
        // add last dwellTime
        ...
        completeShape.geometry.coordinates = sliceResult.globalCoordinates;
