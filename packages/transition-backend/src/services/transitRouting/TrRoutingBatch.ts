...
class TrRoutingBatch {
    ...
    private generateResultFiles = async (): Promise<{
        ...
        for await (const row of resultStream) {
            ...
            const processedResults = await generateFileOutputResults(
                result.data,
                this.job.attributes.data.parameters.transitRoutingAttributes.routingModes,
                {
                    exportCsv: true,
                    exportDetailed: this.job.attributes.data.parameters.transitRoutingAttributes.detailed === true,
                    withGeometries:
                        this.job.attributes.data.parameters.transitRoutingAttributes.withGeometries === true,
                    pathCollection
                }
            );
    ...
    private prepareResultData = async (): Promise<{
        ...
        const resultHandler = createRoutingFileResultProcessor(
            this.job.getJobFileDirectory(),
            this.job.attributes.data.parameters.demandAttributes.configuration,
            this.job.attributes.data.parameters.transitRoutingAttributes,
            this.job.getInputFileName()
        );
    ...
    private getOdTrips = async (): Promise<{
        ...
        const { odTrips, errors } = await parseOdTripsFromCsv(
            join(this.job.getJobFileDirectory(), this.job.getInputFileName()),
            this.job.attributes.data.parameters.demandAttributes.configuration
        );
    ...
    private odTripTask = async (
        ...
        try {
            ...
            const baseRouting = this.job.attributes.data.parameters.transitRoutingAttributes;
            const modes = baseRouting.routingModes || [];
            const routingModesForCalc =
                modes.includes('transit') && !modes.includes('walking') ? [...modes, 'walking' as const] : modes;
            const routingResult = await routeOdTrip(odTrip, {
                trRoutingPort: options.trRoutingPort,
                routing: { ...baseRouting, routingModes: routingModesForCalc },
                reverseOD: false
            });
