...
export class AccessibilityWeightCalculator {
    ...
    private static validateAndPreparePOIs(
        ...
        // Collect all existing valid numeric IDs to avoid conflicts
        const existingIds = new Set<number>();
        validPOIs.forEach((poi) => {
            if (poi.id !== undefined) {
                const poiId = typeof poi.id === 'number' ? poi.id : Number(poi.id);
                if (!isNaN(poiId) && Number.isInteger(poiId)) {
        ...
        // Start generating IDs from max+1, or from 1000000 if no existing IDs (to avoid conflicts with user IDs)
        const maxExistingId = existingIds.size > 0 ? Math.max(...existingIds) : 0;
        let nextAutoId = existingIds.size > 0 ? maxExistingId + 1 : 1000000;

        // Process POIs: use existing numeric IDs or generate new ones
        const validPOIsWithNumericIds = validPOIs.map((poi) => {
            let poiId: number;

            if (poi.id !== undefined) {
                const parsedId = typeof poi.id === 'number' ? poi.id : Number(poi.id);
                if (!isNaN(parsedId) && Number.isInteger(parsedId)) {
                    // Use existing valid numeric ID
                    poiId = parsedId;
                } else {
