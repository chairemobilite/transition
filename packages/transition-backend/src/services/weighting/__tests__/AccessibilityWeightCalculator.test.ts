...
describe('AccessibilityWeightCalculator', () => {
    ...
    describe('calculateWeights', () => {
        test('should throw error when POIs array is empty', async () => {
            ...
            await expect(AccessibilityWeightCalculator.calculateWeights(parametersWithoutPOIs)).rejects.toThrow(TrError);
            await expect(AccessibilityWeightCalculator.calculateWeights(parametersWithoutPOIs)).rejects.toThrow(
                'poisFeatureCollection is required and must contain at least one feature'
            );
        });

        test('should throw error when no places provided', async () => {
            ...
            await expect(AccessibilityWeightCalculator.calculateWeights(parametersWithoutPlaces)).rejects.toThrow(TrError);
            await expect(AccessibilityWeightCalculator.calculateWeights(parametersWithoutPlaces)).rejects.toThrow(
        ...
        test('should return empty results when placesFeatureCollection has no valid features', async () => {
            ...
            const results = await AccessibilityWeightCalculator.calculateWeights(parametersWithInvalidPlaces);
            expect(results).toEqual({});
        });

        test('should throw error when maxBirdDistanceMeters is not positive', async () => {
            const parameters = {
                ...defaultParameters,
                maxBirdDistanceMeters: -100
            };

            await expect(AccessibilityWeightCalculator.calculateWeights(parameters)).rejects.toThrow(TrError);
            await expect(AccessibilityWeightCalculator.calculateWeights(parameters)).rejects.toThrow(
                'maxBirdDistanceMeters must be a positive number'
            );
        });

        test('should throw error when maxNetworkDistanceMeters is not positive', async () => {
            const parameters = {
                ...defaultParameters,
        ...
        test('should throw error when maxTravelTimeSeconds is not positive', async () => {
            const parameters = {
                ...defaultParameters,
        ...
        test('should skip places without geography', async () => {
            ...
            const results = await AccessibilityWeightCalculator.calculateWeights(parameters);
        ...
        test('should return zero weight when no POIs found', async () => {
            ...
            expect(results[place1Id]).toBe(0);
        });
        ...
        test('should calculate weight correctly with single POI', async () => {
            ...
            expect(Object.keys(results)).toHaveLength(1);
            expect(results[place1Id]).toBeDefined();
            // Weight = poiWeight * decayValue = 10.0 * (500^-2) = 10.0 * 0.000004 = 0.00004
            expect(results[place1Id]).toBeCloseTo(10.0 * Math.pow(500, -2), 10);
        });
        ...
        test('should calculate weight correctly with multiple POIs', async () => {
            ...
            expect(results[place1Id]).toBeCloseTo(expectedWeight, 10);
        });
        ...
        test('should include POIs even if network distance exceeds bird distance + 100 tolerance', async () => {
            ...
            expect(results[place1Id]).toBeGreaterThan(0);
        });
        ...
        test('should calculate weights for multiple places', async () => {
            ...
            const results = await AccessibilityWeightCalculator.calculateWeights(defaultParameters);

            expect(Object.keys(results)).toHaveLength(2);
            expect(results[place1Id]).toBeDefined();
            expect(results[place1Id]).toBeGreaterThan(0);
            expect(results[place2Id]).toBeDefined();
            expect(results[place2Id]).toBe(0);
        });
        ...
        test('should use travelTime decayInputType when specified', async () => {
            ...
            expect(results[place1Id]).toBeCloseTo(10.0 * Math.pow(300, -2), 10);
        });
        ...
        test('should call progress callback during calculation', async () => {
            ...
            await AccessibilityWeightCalculator.calculateWeights(defaultParameters, progressCallback);
        ...
        test('should handle decay calculation errors gracefully', async () => {
            ...
            const results = await AccessibilityWeightCalculator.calculateWeights(invalidParameters);
        ...
        test('should use default routing mode when not specified', async () => {
            ...
            await AccessibilityWeightCalculator.calculateWeights(parameters);
    ...
    describe('calculateNodeAccessibilityWeights', () => {
        ...
        test('should handle empty nodeIds array and return empty result', async () => {
            // Empty array means "no nodes" - getPOIsWithinBirdDistanceFromNodes returns {} immediately
            mockedGetPOIsWithinBirdDistanceFromNodes.mockResolvedValue({});
            mockCollection.mockResolvedValue([]);
            ...
            expect(mockCollection).toHaveBeenCalledWith({ nodeIds: [] });
            expect(mockedGetPOIsWithinBirdDistanceFromNodes).toHaveBeenCalledWith(
                [],
                expect.any(Number),
                expect.any(Object)
        ...
        test('should handle non-empty nodeIds array', async () => {
            ...
            expect(result).toHaveProperty(node1Id);
            expect(result[node1Id]).toBeGreaterThanOrEqual(0);
        });
        ...
        test('should return zero weight when nodeIds provided but nodes not found in database', async () => {
            ...
            expect(result).toEqual({ [node1Id]: 0 });
        });
