...
describe('Test accessibility map with results using PostGIS', () => {
    ...
    test('Test one polygon, 2 nodes, with additional properties, POIs, and population', async() => {
        ...
        expect(mockedCensusPopulationCount).toHaveBeenCalledWith(features[0].geometry, 'dsId');
    ...
    test('Test multiple polygons with different durations using PostGIS', async() => {
        ...
        expect(polygonProperties.populationData).toEqual({ population: null, dataSourceAreaRatio: 0 })
    });
