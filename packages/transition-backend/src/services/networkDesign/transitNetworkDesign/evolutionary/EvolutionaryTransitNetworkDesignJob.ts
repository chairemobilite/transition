...
class EvolutionaryTransitNetworkDesignJobExecutor extends TransitNetworkDesignJobWrapper<EvolutionaryTransitNetworkDesignJobType> {
    ...
    private _run = async (): Promise<boolean> => {
        ...
        if (this.job.attributes.internal_data.dataPrepared !== true) {
            ...
            // FIXME: we should not use random here, but just use a single population size.
            // If for some reason we cannot generate enough distinct candidates, then there
            // are too few lines in the chromosome space, or we need another method to warn
            // the user or fallback to less candidates or accept duplicate candidates.
        ...
        // FIXME: we should not save the results into the json blob since they are in the db already.
    ...
    // call this on-demand to get partial results (TODO)
