...
async function buildInputFilesForEvolutionaryJob(
    ...
    if (config.nodeWeighting?.weightingEnabled && weightingCsv) {
        try {
...
export const getParametersFromTransitNetworkDesignJob = async (
    ...
    if (clonedParameters.simulationMethod.type === 'OdTripSimulation') {
        ...
        const nodeWeighting = config.nodeWeighting;
        if (nodeWeighting?.weightingFileAttributes?.fileAndMapping) {
            nodeWeighting.weightingFileAttributes.fileAndMapping.csvFile = {
                location: 'job',
                jobId,
                fileKey: 'nodeWeight'
            };
