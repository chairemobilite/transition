...
**Load factor penalty** (optional):

$$\mu_ {\alpha_c}^{(k)} = \begin{cases} \mu_ {\alpha_c} \times (\alpha_ {c,k} + 1.0) & \text{if } \alpha_ {c,k} \text{ is defined and not NaN} \\ 1.0 & \text{otherwise} \end{cases}$$
...
$$C_ {\text{reliability}}^{(k)} = \begin{cases} \mu_ {R} \times R_k & \text{if } R_k \text{ is defined and not NaN} \\ 0 & \text{otherwise} \end{cases}$$

Where:
- $R_k$: reliability ratio for leg $k$ (ratio of on-time arrivals/departures, value between 0.0 and 1.0)
- $\mu_ {R}$: reliability penalty factor
...
& + \sum_ {k=0}^{n_ {tr}} \left[ \mu_ {R} \times R_k \right] \quad \text{(if reliability data available)}
\end{aligned}$$
...
- **Load factor**: Added to 1.0 before multiplying with penalty factor to ensure the result is always $\geq 1.0$
- **Reliability ratio**: On-time performance ratio (0.0 to 1.0), where higher values indicate better reliability
- **Optional components**: Load factor and reliability ratio are ignored when undefined or NaN
...
| Reliability penalty | $\mu_ {R}$ | Additive |
