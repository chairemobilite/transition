...
describe('Timepoint to checkpoint mapping', () => {
    ...
    test('Creates checkpoints from mixed timepoint values', async () => {
        ...
        expect(createdPathAttribs.data.segmentTimesCheckpoints).toEqual([
            { fromNodeId: nodeId1, toNodeId: nodeId3, fromNodeOccurrence: 0, toNodeOccurrence: 0 },
            { fromNodeId: nodeId3, toNodeId: nodeId4, fromNodeOccurrence: 0, toNodeOccurrence: 0 }
        ]);
    });
    ...
    test('Creates checkpoints from timepoints without shape', async () => {
        ...
        expect(createdPathAttribs.data.segmentTimesCheckpoints).toEqual([
            { fromNodeId: nodeId1, toNodeId: nodeId4, fromNodeOccurrence: 0, toNodeOccurrence: 0 }
        ]);
    });
});
