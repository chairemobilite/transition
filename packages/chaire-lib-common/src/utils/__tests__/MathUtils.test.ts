...
describe('ceilToPositiveInteger', () => {
    test.each([
        [540, 540],
        [540.1, 541],
        [540.7, 541],
    ...
    test.each([[-5], [-0.1], [NaN], [Infinity], [-Infinity]])(
        'should throw for invalid input %p',
        (input) => {
            expect(() => MathUtils.ceilToPositiveInteger(input as any)).toThrow(
                'ceilToPositiveInteger: Value must be a finite number and >= 0'
            );
        }
    );
});