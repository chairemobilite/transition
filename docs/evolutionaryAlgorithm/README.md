...
These instructions explain how to run the evolutionary algorithm in Transition. It is still very experimental and require manual manipulations. This algorithm picks from a set of many transit paths and schedules, the subset that offer the "best network performances", given certain parameters.
...
*Note*: Until [this issue](https://github.com/chairemobilite/transition/issues/1286) is fixed, you may need to update the user's preferences directly in the database to make the section appear in the interface.
...
The chromosomes can also be compared using a mix of those methods, the weight of each can be set at runtime. For example, it would be possible to give a fitness mark that would correspond 75% to the accessibility map method and 25% OD trips, to give more weight to accessibility that current trip routings.
...
First, a `polygon.geojson` file containing the polygon for which to import the data needs to be available in the data source directory. [geojson.io](https://geojson.io) can be used to create one.


The following steps need to be done to prepare the data: 

*Note*: all those steps can be done non-interactively. But in the absence of a proper way for tasks to announce their parameters and the lack of documentation for it, the current approach is to read to code and find out (and update this documentation!) or run it interactively.
...
It will display the simulation configuration and invite you to edit them for this specific run, just clicking enter should be good if the parameters are correct. 
...
For the `Accessibility maps` method, it asks for the data source to use and the maximum sample ratio to use from that datasource. By default, it will stop after a while, when the result variability is very low, but as long as variability changes, it will test places until that ratio is reached.
